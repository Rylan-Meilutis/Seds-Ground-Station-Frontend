...
def _frontend_script_dir() -> Path:
    return Path(__file__).resolve().parent


def _resolve_frontend_dir() -> Path:
    script_dir = _frontend_script_dir()

    if (script_dir / "Cargo.toml").exists() and (script_dir / "Dioxus.toml").exists():
        return script_dir

    nested_frontend = script_dir / "frontend"
    if (nested_frontend / "Cargo.toml").exists() and (nested_frontend / "Dioxus.toml").exists():
        return nested_frontend

    raise FileNotFoundError(
        "Could not locate the frontend directory relative to build.py. "
        "Expected Cargo.toml and Dioxus.toml next to build.py or under ./frontend/."
    )


def _workspace_root(frontend_dir: Path) -> Path:
    parent = frontend_dir.parent
    cargo_toml = parent / "Cargo.toml"
    if cargo_toml.exists():
        data = _load_toml_file(cargo_toml)
        if isinstance(data.get("workspace"), dict):
            return parent
    return frontend_dir


def _target_root(frontend_dir: Path) -> Path:
    return _workspace_root(frontend_dir) / "target"
...
def _stage_app_bundle_from_dx(
    ...
    target_root = _target_root(frontend_dir) / "dx" / pkg_name
...
def _resolve_makensis() -> Optional[str]:
    ...
    repo_root = _workspace_root(_resolve_frontend_dir())
    search_roots.extend([
        repo_root / ".tauri",
        repo_root / "dx",
...
def _windows_bundle_search_roots(
    ...
    target_root = _target_root(frontend_dir)
...
def _stage_linux_app_payload(
    ...
    description = _read_workspace_description(_workspace_root(frontend_dir))
...
def _generated_android_app_dir(frontend_dir: Path, debug_mode: bool) -> Path:
    profile = "debug" if debug_mode else "release"
    pkg_name = _frontend_package_name(frontend_dir)
    return _target_root(frontend_dir) / "dx" / pkg_name / profile / "android" / "app"
...
def _clear_dioxus_bundle_identity_cache(
    ...
    if removed:
        rel = ", ".join(str(p.relative_to(_workspace_root(frontend_dir))) for p in removed)
...
def _android_signing_settings(frontend_dir: Path) -> dict[str, str]:
    repo_root = _workspace_root(frontend_dir)
    defaults = {
...
def _configure_android_signing(frontend_dir: Path, project_dir: Path) -> bool:
    ...
    if not keystore_path.is_absolute():
        keystore_path = (_workspace_root(frontend_dir) / keystore_path).resolve()
...
def patch_generated_android_project(frontend_dir: Path, debug_mode: bool) -> Path:
    ...
    patch_script = _workspace_root(frontend_dir) / "scripts" / "patch_android_webview_logging.py"
    if debug_mode and patch_script.exists():
        run([sys.executable, str(patch_script)], cwd=_workspace_root(frontend_dir))
    return project_dir
...
def build_android_universal_apk(frontend_dir: Path) -> Path:
    ...
    if aab is None:
        raise FileNotFoundError(f"No Android .aab artifact found in {dist_dir(frontend_dir)}")
...
def _ensure_wasm_bindgen_cli(frontend_dir: Path, env: Optional[dict[str, str]]) -> Optional[Path]:
    ...
    if installed_path is None:
        ...
        run(
            ...
            cwd=_workspace_root(frontend_dir),
            env=env,
        )
    else:
        print(
...
def _clear_dx_web_cache(frontend_dir: Path) -> None:
    ...
    """
    workspace_root = _workspace_root(frontend_dir)
    target_dirs = {
        frontend_dir / "target",
...
def _display_build_script_path() -> str:
    frontend_dir = _resolve_frontend_dir()
    script_dir = _frontend_script_dir()
    if script_dir == frontend_dir:
        return "./build.py"
...
def print_usage(exit_code: int = 1) -> None:
    ...
    print(f"  {build_script} ios_sign [debug] [existing]")
    print(f"  {build_script} ios_dist_sign [debug] [existing]")
    print(
        f"  {build_script} android_install [debug] [existing]   # build/install APK to a connected emulator/device")
    print(
        f"  {build_script} android_screenshot [debug] [existing] [screenshot_delay=<seconds>] ["
        "screenshot_out=<path>] [screenshot_name=<name>]")
    print(
