...
const server=http.createServer((req,res)=>{
  ...
  if(pathname.startsWith('/api/')){
    ...
    const data=pathname==='/api/auth/session'?session:pathname==='/api/layout'?example('layout.full'):pathname==='/api/live_streams'?{...example('live-streams'),can_manage_stream:session.roles.includes('stream_master'),program_url:''}:pathname==='/api/vehicle'?{}:[];
...
try{
  ...
  await choose();await page.getByRole('button',{name:'My Dashboard',exact:true}).click();
  ...
  console.log('PASS: stream manager permissions, card editing, live pins, per-user persistence, hidden tabs, iframe session handoff/lifetime, desktop and mobile tab picker');
